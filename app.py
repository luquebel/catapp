# ==============================================================
# Autor: Luis J. Luque B.
# email: LuqueBeL@gmail.com
# ==============================================================
...
# A continuacion tenemos a la funcion gbarras(df), la cual genera un grafico de barras, resaltando la pelicula con mejor calificacion del publico segun el proveedor de streaming seleccionado, cuya puntuacion IMDb sea mayor a 6 puntos.

def gbarras(df):
    dataf=df[(df['IMDb']>6) & (df[opcion]==1)][['IMDb','Title','Genres']].sort_values(by='IMDb',ascending=False).head()
    
    mayorcat=dataf['Title'].head(1).max(axis = 0)
    tipocategoria=dataf['Genres'].head(1).max(axis = 0)
    ...
    barra= alt.Chart(dataf).mark_bar().encode( 
           ...
           alt.value('#154734'),     
           alt.value('lightgrey'))      
           ).properties(height=300,width=650,title="Tipo de Categoria de la Pelicula: " + tipocategoria) 
...
diversos aspectos de entretenimiento por Streaming, como por ejemplo:
    ...
    • ¿Cuál categoría es la mas ofertada por proveedor de entretenimiento
       via streaming (Netflix, Amazon Prime, HULU, Disney+) ?

    • ¿Cuál categoría recibe mejores criticas de los expertos del Cine, 
       segmentados por proveedor de entretenimiento?
    
    • ¿Cuál es la película con la mejor critica del publico en general, 
       segmentados por proveedor de entretenimiento?
    
    • ¿Cuál es la película con mejor critica de los expertos del Cine 
       que se visualiza por proveedor de streaming?
    
    • ¿Cuál es el director con mejor critica, tanto del publico en 
       general, como de los expertos del Cine por proveedor de streaming?
...
#Se establecen los parametros iniciales de configuracion de la pagina
st.set_page_config(
...
st.sidebar.write("Autor: Luis J. Luque B.")
st.sidebar.write("Email: LuqueBeL@gmail.com")
...
if pagina== "Home Page":
    ...
    st.success("¿Cuál es la película con la mejor critica del publico en general, segmentados por proveedor de entretenimiento?")
    opcion = st.selectbox('Seleccione',('','Netflix','Hulu','Prime Video','Disney+'),index=0)
    if opcion != "": 
